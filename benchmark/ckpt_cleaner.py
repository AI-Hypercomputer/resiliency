...
logging.basicConfig(
    level=logging.INFO,
    format=(
        f'[CheckpointCleaner@n{os.getenv("NODE_RANK")}]%(asctime)s -'
...
def parse_args():
  ...
  parser.add_argument(
      "--local-ckpt-dir-prefix",
      required=True,
      help="Base directory for local checkpoints (required)",
  )
  parser.add_argument(
      "--persistent-ckpt-dir-prefix",
      default=None,
      help="Base directory for persistent checkpoints.",
  )
  parser.add_argument(
      "--job-identifier",
      required=True,
...
def cleanup_non_matching_jobs(checkpoint_dir_prefix, job_identifier):
  ...
  logger.info(
      f"Cleaning up non-matching job folders under {checkpoint_dir_prefix}..."
  )
...
def cleanup_unfinished_checkpoints(checkpoint_path):
  ...
  logger.info(f"Cleaning up unfinished checkpoints under {checkpoint_path}...")
  checkpoint_path = Path(checkpoint_path)
...
def process_checkpoints(checkpoint_path, num_checkpoints_to_keep):
  ...
  complete_checkpoints, unfinished_checkpoints = find_checkpoints(
      checkpoint_path
  )
  ...
  logger.info(f"Under folder {checkpoint_path}: ")
...
def main():
  ...
  if args.persistent_ckpt_dir_prefix:
    persistent_ckpt_dir = os.path.join(
        args.persistent_ckpt_dir_prefix, job_identifier, "checkpoint"
    )
    # only node0 run clean on persistent storage
    if int(os.getenv("NODE_RANK", -1)) == 0:
      cleanup_unfinished_checkpoints(persistent_ckpt_dir)

  local_ckpt_dir_prefix = args.local_ckpt_dir_prefix
  local_ckpt_dir = os.path.join(
      local_ckpt_dir_prefix, job_identifier, "checkpoint"
  )
  ...
  cleanup_non_matching_jobs(local_ckpt_dir_prefix, job_identifier)
  cleanup_unfinished_checkpoints(local_ckpt_dir)

  # Main cleanup loop
  while True:
    # Check if CHECKPOINT_DIR exists and is not empty
    checkpoint_path = Path(local_ckpt_dir)
