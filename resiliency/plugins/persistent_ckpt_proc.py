...
from resiliency.third_party.megatron.async_utils import AsyncCallsQueue, debug_time
from resiliency.utils import get_resiliency_logger

from ._ckpt_utils import TorchCompatiblePersistentAsyncCaller
...
class PersistentCheckpointProcessIO(AsyncFinalizableCheckpointIO):
  ...
  def save_checkpoint(
    ...
    Requires the underlying checkpoint_io.save_checkpoint to return an
    AsyncRequest.
    ...
    Applies underlying checkpoint_io finalize callback first, then the external
      one (postfix order).
    """
    ...
    logger.debug(f"Scheduled an async call #{call_idx}")
