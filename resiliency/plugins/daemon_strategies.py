...
from resiliency.third_party.megatron.async_utils import AsyncRequest
...
class DaemonFileSystemWriterAsync(FileSystemWriterAsync):
  ...
  def prepare_write_data(self, plan: SavePlan, planner: SavePlanner) -> None:
    ...
    Args:
        plan (SavePlan): save plan generated by the PyT Distributed compatible
          planner
        planner (SavePlanner): save planner used to resolve the bytes and tensor
          data
    ...
    item_buckets = _split_by_size_and_type(
        bins, plan.items, self.separation_hint
    )
  ...
  def get_save_function_and_args(self) -> Tuple[Optional[Callable], Tuple]:
    ...
    Allows the external caller to apply the save function synchronously or
    asynchronously.
  ...
  def write_preloaded_data_subproc(
    ...
    Starts predefined number of processes and uses 2 queues to make sure the
    results
    are complete:
    ...
    To prevent this, we disable the GC explicitly for this function with
    _disable_gc.
  ...
  def write_preloaded_data(
    ...
    Args:
        ...
        results_queue (mp.Queue): queue to return the write results to the proxy
          checkpoint process.
  ...
  def retrieve_write_results(self) -> List[WriteResult]:
    ...
    if len(write_results) != len(self.write_buckets):
      raise RuntimeError(
          f"Incomplete worker results (expected {len(self.write_buckets)}, got"
          f" {len(write_results)}. This probably indicates a worker failure."
      )
