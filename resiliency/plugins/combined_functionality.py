...
This file combines the checkpoint functionality from `in_cluster_local_ckpt.py`
and `min_ckpt_overhead.py`.
TODO: Better utilize abstractions to avoid redundant code usage.
"""
...
from resiliency.third_party.torch._async_process_executor import _CheckpointRequestIdentifier
...
from ._ckpt_utils import (
    TorchCompatiblePersistentAsyncCaller,
...
from .daemon_strategies import (
    DaemonFileSystemWriterAsync,
    DaemonTorchDistSaveShardedStrategy,
)
from .min_ckpt_overhead import MinCkptOverheadCheckpointIO
...
class CombinedCheckpointIO(MinCkptOverheadCheckpointIO):
  ...
  @debug_time("CombinedCheckpointIO.load_checkpoint")
  def load_checkpoint(
      self,
      path: _PATH,
    ...
    if self.save_ckpt_format == "zarr" and self.load_directly_on_device:
      ...
      sharded_strategy = CombinedCheckpointLoadStrategy(
...
class CombinedCheckpointSaveStrategy(DaemonTorchDistSaveShardedStrategy):
  ...
  def async_save(
    ...
    writer = CombinedCheckpointFileSystemWriter(
        checkpoint_dir,
...
class CombinedCheckpointLoadStrategy(TorchDistLoadShardedStrategy):
...
class CombinedCheckpointFileSystemWriter(DaemonFileSystemWriterAsync):
