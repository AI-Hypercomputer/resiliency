...
class ModelCheckpoint(PTLModelCheckpoint):
  ...
  Args:
      ...
      save_last: When ``True``, saves a `*-last` copy whenever a checkpoint file
        gets saved.
      save_top_k: When ``True``, saves the top-k checkpoints according to
        ``monitor``.
      save_weights_only:  if ``True``, then only the model's weights will be
        saved. Optimizer states will be omitted from all checkpoints.
      mode: One of {min, max}. Whether the objective is to minimize or maximize
        the monitored quantity.
      every_n_epochs: Number of epochs between checkpoints.
      every_n_train_steps: Number of train steps between checkpoints.
      train_time_interval: After each interval, monitor checkpoints. Not to be
        used with ``every_n_epochs`` or ``every_n_train_steps``.
      ...
      save_context_on_train_end: Whether to dump the artifacts on_train_end
        regardless of whether ``always_save_context`` is ``True``.
  ...
  def __init__(
    ...
    # Define priority for each checkpoint callback in case there are multiple.
    # When there are multiple, highest priority callback will run.
    self.priority = priority

    # Define whether this callback saved checkpoint this step.
    self._just_saved_ckpt = False
    ...
    self.is_checkpoint_file_handler = get_is_checkpoint_file_handler(
  ...
  def on_train_start(self, trainer, pl_module):
    ...
    if self.preprocess_files and self.is_checkpoint_file_handler:
  ...
  def nemo_topk_check_previous_run(self):
    try:
      ...
      raise AttributeError(
          "Lightning's ModelCheckpoint was updated. NeMo's ModelCheckpoint will"
          " need an update."
      )
    self.best_k_models = {}
    self.kth_best_model_path = ""
  ...
  def format_checkpoint_name(self, *args, **kwargs) -> str:
    file_name = super().format_checkpoint_name(*args, **kwargs)
    return file_name.removesuffix(self.FILE_EXTENSION)
  ...
  def on_train_end(self, trainer, pl_module):
    ...
    if self.save_last and trainer.val_check_interval != 0:
      ...
      if (
          ...
          and self.is_checkpoint_file_handler
      ):
  ...
  def format_checkpoint_unfinished_marker_path(
    ...
    If the marker file exists, corresponding checkpoint is considered
    unfinished/incomplete.
    NOTE: Marker path for the EMA checkpoint part is the same as for the
    original checkpoint.

    Args:
        checkpoint_path: Path to the checkpoint file or dir. Does not need to
          exist.
  ...
  def set_checkpoint_unfinished_marker(
      checkpoint_path: Union[Path, str],
      barrier_after=False,
      is_checkpoint_file_handler=False,
  ) -> None:
    """Marks given checkpoint as unfinished.

    Args:
        checkpoint_filepath: Path to the checkpoint file or dir. Does not need
          to exist.
        barrier_after: Synchronize ranks after writing the marker file. Defaults
          to False.
    """
    if is_checkpoint_file_handler:
  ...
  def _save_checkpoint(
    ...
    self.set_checkpoint_unfinished_marker(
        filepath,
        barrier_after=True,
        is_checkpoint_file_handler=self.is_checkpoint_file_handler,
    )
    ...
    # Define storage options for the checkpoint.
    storage_options = {"is_persistent_storage": self.is_persistent_storage}
    ...
    if ema_callback is not None:
      ...
      with ema_callback.save_ema_model(trainer):
        ...
        if trainer.is_global_zero:
          for logger in trainer.loggers:
            logger.after_save_checkpoint(proxy(self))
      self.remove_checkpoint_unfinished_marker(
          filepath,
          barrier_before=True,
          is_checkpoint_file_handler=self.is_checkpoint_file_handler,
      )
      ...
      if self.always_save_context and self.is_checkpoint_file_handler:
        TrainerContext.from_trainer(trainer).io_dump(
  ...
  def _get_finalize_save_checkpoint_callback(
    ...
    def _cb():
      logging.debug(
          f"Finalize callback called for step {global_step}, filepath"
          f" {filepath}"
      )
      self._last_checkpoint_saved = filepath
      ...
      if os.getenv("RANK") == "0":
        goodput_logging.log_event(
            goodput_event.CHECKPOINT_SAVED, step=global_step
        )

      logging.info(
          f"Checkpoint save for step {global_step} ({filepath}) finalized"
          " successfully."
      )
      ...
      if self.enable_high_scale_ckpt and self.is_checkpoint_file_handler:
        ...
        logging.info(
            f"rename folder for high scale ckpt: from {ckpt_path} to"
            f" {high_scale_ckpt_path}"
        )
        os.rename(ckpt_path, high_scale_ckpt_path)
  ...
  def _has_ema_ckpts(self, checkpoints: Iterable[Path]) -> bool:
    return any(
        self._is_ema_filepath(checkpoint_path)
  ...
  @staticmethod
  def _remove_unfinished_checkpoints(
      checkpoint_dir: Union[Path, str], is_checkpoint_file_handler=False
  ) -> None:
  ...
  def _should_remove_checkpoint(
    ...
    A checkpoint won't be deleted if any of the cases apply: - The previous
    checkpoint is the same as the current checkpoint (means the old was already
    overwritten by new) - The previous checkpoint is not in the current
    checkpoint directory and the filesystem is local - The previous checkpoint
    is the checkpoint the Trainer resumed from and the filesystem is local
  ...
  def _should_skip_saving_checkpoint(self, trainer: "pl.Trainer") -> bool:
    """Checks if the checkpoint should be skipped.

    A checkpoint won't be saved if any of the cases apply: - The Trainer is
    running in fast_dev_run mode - The Trainer is running in debug mode - The
    Trainer is running in test mode - The Trainer is running in prediction mode
    """
    ...
    if len(ckpt_callbacks) > 1:
      # Skip checkpointing if another checkpoint callback with higher priority
      # already saved the checkpoint for this step.

      for ckpt_callback in ckpt_callbacks:
        if ckpt_callback.priority > self.priority:
          if ckpt_callback._just_saved_ckpt:
            logging.debug(
                "Skipping saving checkpoint for step"
                f" {trainer.global_step} because another checkpoint callback"
                f" with higher priority ({ckpt_callback.priority}) already"
                " saved this step."
            )
            return True
