...
class MegatronCommOverlapCallback(pl.Callback):
  ...
  Args:
      ...
      tp_comm_overlap_cfg (TransformerLayerTPOverlapCfg): Tensor parallel
        overlap config
      ...
      overlap_grad_reduce (bool): Overlap data parallel gradient reduction with
        compute
      overlap_param_gather (bool): Overlap data parallel parameter gather with
        compute
      overlap_param_gather_with_optimizer_step (bool): Overlap data parallel
        parameter gather optimizer step
      align_param_gather (bool): Align data parallel parameter gather across
        virtual pipeline chunks
      bucket_size (int): The DDP bucket size, controls the data parallel overlap
        granularity
      defer_embedding_wgrad_compute (bool): Overlap wgrads with the pipeline
        drain bubble for the last pipeline stage
      wgrad_deferral_limit (int): Limit of how many outstanding wgrads may be
        overlapped with the pipeline drain bubble
  ...
  def _get_model_comm_overlap_cfgs(
    ...
    if self.user_comm_overlap_cfg.tp_comm_overlap is True:
      if parallelism_cfg.tensor_model_parallel_size < 2:
        logging.warning(
            "Disabling tensor parallel communication overlap due to TP size"
            " < 2."
        )
        ...
        logging.warning(
            "Disabling tensor parallel communication overlap due to TE not"
            " detected."
        )
        self.user_comm_overlap_cfg.tp_comm_overlap = False
  ...
  def _get_optimizer_overlap_cfgs(
      self, parallelism_cfg: ParallelismConfig
  ) -> _CommOverlapConfig:
  ...
  def setup(
    ...
    ), "MegatronCommOverlapCallback requires MegatronStrategy"
    parallelism_cfg = trainer.strategy.parallelism

    if hasattr(trainer.model, "config") and isinstance(
        trainer.model.config, ModelParallelConfig
    ):
      ...
      if trainer.model.config.tp_comm_overlap:
        ...
        self.tp_comm_bootstrap_backend = (
            comm_overlap_cfg.tp_comm_bootstrap_backend
        )
    ...
    if (
        ...
        and isinstance(
            trainer.strategy.ddp_config, DistributedDataParallelConfig
        )
  ...
  def _init_te_userbuffers(self, model_parallel_cfg: ModelParallelConfig):
    ...
    if self.tp_comm_overlap_cfg is None:
      logging.warning(
          "Tensor parallel overlap: No overlap config provided. Initializing TP"
          " comm overlap with the default config."
      )
    else:
    ...
    try:
      ...
      raise Exception(
          "Tensor parallel overlap: userbuffer initialization failed with"
          f" {error}"
      )
