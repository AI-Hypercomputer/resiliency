...
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.multiprocessing import start_processes
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.multiprocessing.api import LogsSpecs
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.multiprocessing.api import (
    LogsSpecs,
    PContext,
    SignalException,
    Std,
...
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.multiprocessing.errors import ChildFailedError, record
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.rendezvous import RendezvousParameters
from nvidia_resiliency_ext.fault_tolerance._torch_elastic_compat.rendezvous import registry as rdzv_registry
...
from nvidia_resiliency_ext.fault_tolerance.utils import terminate_mp_processes
...
logging.basicConfig(
    level=logging.INFO,
    format=(
        f"[%(asctime)s] [%(levelname)s] [ft_launcher@{socket.gethostname()}]"
        " %(message)s"
    ),
)
...
class RankCachingRdzvHandlerWrapper:
  ...
  same as in the last redezvous. It just caches last group rank obtained from
  rdzv handler
  and return it for every next redezvous as long as group world size is the
  same.
  """
...
class LocalElasticAgent(SimpleElasticAgent):
  ...
  Logs are written to the specified log directory. Each log line will be by
  default
  ...
  ``${role_name}, ${local_rank}, ${rank}``. For example, to prefix each log line
  with
  global rank instead of the local rank, set ``log_line_prefix_template =
  "[${rank}]:``.
  ...
  def setup_goodput_client(self, envs: Dict[int, Dict[str, str]]) -> None:
    for worker_env in envs.values():
      try:
        rank = int(worker_env["RANK"])
  ...
  def shutdown_goodput_client(self):
    pass
    # for client in self._rank_to_gclient.values():
    #    client._worker.
  ...
  def _start_workers(self, worker_group: WorkerGroup) -> Dict[int, Any]:
    ...
    if self.use_supervisor:
      self.setup_goodput_client(envs=envs)
    ...
    self._children_pgids = {
        os.getpgid(p) for p in self._pcontext.pids().values()
    }
  ...
  def _monitor_workers(self, worker_group: WorkerGroup) -> RunResult:
    logger.debug(f"_monitor_workers")
    ...
    if worker_pids != pc_pids:
      ...
      logger.debug(f"_monitor_workers unknown")
    ...
    if result:
      if result.is_failed():
        ...
        for local_rank, failure in result.failures.items():
          ...
          if (
              self._rank_to_gclient
              and self._rank_to_gclient[worker.global_rank] is not None
          ):
            from supervisor import DeviceState

            self._rank_to_gclient[worker.global_rank].state = DeviceState.FAILED
            self._rank_to_gclient[worker.global_rank].send_heartbeat()
        ...
        for local_rank, ret_val in result.return_values.items():
          ...
          if (
            ...
            self._rank_to_gclient[worker.global_rank].state = (
                DeviceState.COMPLETE
            )
            self._rank_to_gclient[worker.global_rank].send_heartbeat()
        logger.info(f"job succeeede")
        return RunResult(
            state=WorkerState.SUCCEEDED,
            return_values=workers_ret_vals,
        )
    else:
      logger.debug(f"check healthy")
      from supervisor import DeviceState

      for rank, client in self._rank_to_gclient.items():
        if client is None:
          continue
        self._rank_to_gclient[rank].state = DeviceState.RUNNING
...
class LaunchConfig:
  ...
  Args:
      min_nodes: Minimum amount of nodes that the user function will be launched
        on. Elastic agent ensures that the user function start only when the
        min_nodes amount enters the rendezvous.
      max_nodes: Maximum amount of nodes that the user function will be launched
        on.
      nproc_per_node: On each node the elastic agent will launch this amount of
        workers that will execute user defined function.
      rdzv_backend: rdzv_backend to use in the rendezvous (zeus-adapter, etcd).
      rdzv_endpoint: The endpoint of the rdzv sync. storage.
      rdzv_configs: Key, value pair that specifies rendezvous specific
        configuration.
      rdzv_timeout: Legacy argument that specifies timeout for the rendezvous.
        It is going to be removed in future versions, see the note below. The
        default timeout is 900 seconds.
      run_id: The unique run id of the job (if not passed a unique one will be
        deduced from run environment - flow workflow id in flow - or auto
        generated).
      ...
      start_method: The method is used by the elastic agent to start the workers
        (spawn, fork, forkserver).
      metrics_cfg: configuration to initialize metrics.
      local_addr: address of the local node if any. If not set, a lookup on the
        local machine's FQDN will be performed.
      local_ranks_filter: ranks for which to show logs in console. If not set,
        show from all.
...
class elastic_launch:
  """Launches an torchelastic agent on the container that invoked the entrypoint.
      ...
      1. Pass the ``entrypoint`` arguments as non ``kwargs`` (e.g. no named
      parameters)/
...
def _get_entrypoint_name(
  ...
  """Retrieve entrypoint name with the rule: 1.

  If entrypoint is a function, use ``entrypoint.__qualname__``. 2. If entrypoint
  is a string, check its value:

      2.1 if entrypoint equals to ``sys.executable`` (like "python"), use the
      first element from ``args``
          which does not start with hifen letter (for example, "-u" will be
          skipped).
...
def _get_addr_and_port(
  ...
  if not endpoint:
    raise ValueError(
        "Endpoint is missing in endpoint. Try to add --master-addr and"
        " --master-port"
    )
  master_addr, master_port = parse_rendezvous_endpoint(
      endpoint, default_port=-1
  )
  if master_port == -1:
    raise ValueError(
...
def launch_agent(
  ...
  if not config.run_id:
    ...
    logger.warning(
        "config has no run_id, generated a random run_id: %s", run_id
    )
  ...
  try:
    ...
    if agent.any_rank_failed():
      ...
      logger.info(
          "All ranks exited gracefully. Launcher exiting without error."
      )
  except Exception:
    events.record(agent.get_event_failed())
    raise
...
def get_args_parser() -> ArgumentParser:
  ...
  parser.add_argument(
      ...
      default="1:1",
      help=(
          "Number of nodes, or the range of nodes in form"
  ...
  parser.add_argument(
      ...
      action=check_env,
      help=(
          "Start a local standalone rendezvous backend that is represented by a"
          " C10d TCP store on a free port. Useful when launching single-node,"
          " multi-worker job. If specified --rdzv-backend, --rdzv-endpoint,"
          " --rdzv-id are auto-assigned and any explicitly set values are"
          " ignored."
      ),
  )
  ...
  parser.add_argument(
      ...
      default=1800,
      help=(
          "Interval, in seconds, between initial SIGTERM and rank termination"
          " with SIGKILL."
      ),
  )
  parser.add_argument(
  ...
  parser.add_argument(
      ...
      action=check_env,
      help=(
          "Change each process to interpret the launch script as a Python"
  ...
  parser.add_argument(
      ...
      default="0",
      help=(
          "Redirect std streams into a log file in the log directory (e.g. [-r"
          " 3] redirects both stdout+stderr for all workers, [-r 0:1,1:2]"
          " redirects stdout for local rank 0 and stderr for local rank 1)."
      ),
  )
  parser.add_argument(
      "-t",
      ...
      default="0",
      help=(
          "Tee std streams into a log file and also to console (see --redirects"
          " for format)."
      ),
  )
  ...
  parser.add_argument(
      ...
      default="",
      help=(
          "Only show logs from specified ranks in console (e.g."
          " [--local_ranks_filter=0,1,2] will only show logs from rank 0, 1 and"
          " 2). This will only apply to stdout and stderr, not tolog files"
          " saved via --redirect or --tee"
      ),
  )
  ...
  parser.add_argument(
      ...
      action=env,
      help=(
          "Address of the master node (rank 0) that only used for static"
          " rendezvous. It should be either the IP address or the hostname of"
          " rank 0. For single node multi-proc training the --master-addr can"
  ...
  parser.add_argument(
      ...
      action=env,
      help=(
          "Port on the master node (rank 0) to be used for communication during"
  ...
  parser.add_argument(
      ...
      action=env,
      help=(
          "Address of the local node. If specified, will use the given address"
          " for connection. Else, will look up the local node address instead."
  ...
  parser.add_argument(
      ...
      type=str,
      help=(
          "torchrun.logs_specs group entrypoint name, value must be type of"
          " LogsSpecs. Can be used to override custom logging behavior."
      ),
  )
  ...
  parser.add_argument(
      ...
      action=env,
      help=(
          "Path to a YAML file that contains Fault Tolerance pkg config"
          " (`fault_tolerance` section). NOTE: config items from the file can"
          " be overwritten by `--ft-param-*` args."
      ),
  )
  ...
  parser.add_argument(
      ...
      default=None,
      help=(
          "Part of Fault Tolerance pkg config (initial_rank_heartbeat_timeout)."
      ),
  )
  ...
  parser.add_argument(
      ...
      type=str,
      help=(
          "Full path to the (single GPU) training program/script to be launched"
...
def get_use_env(args) -> bool:
  ...
  ``use_env`` is a legacy argument, if ``use_env`` is False, the ``--node-rank``
  argument will be transferred to all worker processes. ``use_env`` is only used
  by the ``torch.distributed.launch`` and will be deprecated in future releases.
  """
...
def _get_logs_specs_class(logs_specs_name: Optional[str]) -> Type[LogsSpecs]:
  ...
  Raises ValueError when entrypoint for `logs_spec_name` can't be found in
  entrypoints.
  """
  logs_specs_cls = None
  if logs_specs_name is not None:
    ...
    if logs_specs_cls is None:
      raise ValueError(
          "Could not find entrypoint under"
          f" 'torchrun.logs_specs[{logs_specs_name}]' key"
      )

    logging.info(
        "Using logs_spec '%s' mapped to %s",
        logs_specs_name,
        str(logs_specs_cls),
    )
  else:
...
def config_from_args(
  ...
  if (
    ...
    logger.warning(
        ...
        " rdzv_endpoint is not specified."
    )
  ...
  try:
    ...
    if args.ignore_missing_fault_tol_cfg:
      logger.warning(
          f"Could not load FT config from '{args.fault_tol_cfg_path}' or read"
          " from CLI args. Will use default FT settings."
      )
      fault_tol_cfg = FaultToleranceConfig()
    else:
  ...
  if args.local_ranks_filter:
    try:
      ...
      raise Exception(
          "--local_ranks_filter must be a comma-separated list of integers e.g."
          " --local_ranks_filter=0,1,2"
      ) from e
