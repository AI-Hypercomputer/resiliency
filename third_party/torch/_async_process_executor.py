...
# All rights reserved.
#
# This file is copied from torch.distributed.checkpoint._async_process_executor.py with no changes.
#
# PyTorch is licensed under the BSD-style license; see
# https://github.com/pytorch/pytorch/blob/main/LICENSE for details.
...
from .logger import _dcp_method_logger, _init_logger
...
class _ProcessGroupInitInfo:
  ...
  def __init__(self, process_group: Optional[dist.ProcessGroup] = None):
    ...
    self.tcp_store_master_addr, self.tcp_store_master_port = (
        dist_wrapper.broadcast(
...
class _ProcessBasedAsyncCheckpointExecutor(_AsyncCheckpointExecutor):
  ...
  def execute_save(
    ...
    initialization. Therefore, the first async checkpoint request will take
    longer to complete.

    - Process initialization can have significant overhead, dominated by latency
    for all ranks to spawn
    a background process + process group initialization in the background
    process.
    """
