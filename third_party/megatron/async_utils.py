...
# This file is copied from megatron.core.dist_checkpointing.strategies.async_utils.py
# with the following changes:
# - Added `logger_stack` and `debug_time` context managers from megatron.core.dist_checkpointing.utils.py

# Megatron-LM is licensed under the Apache 2.0 License; see
# https://github.com/NVIDIA/Megatron-LM/blob/main/LICENSE for details.
...
class AsyncRequest(NamedTuple):
  ...
  Args:
      async_fn (Callable, optional): async function to call. None represents
        noop.
      async_fn_args (Tuple): args to pass to `async_fn`.
      finalize_fns (List[Callable]): list of functions to call to finalize the
        request. These functions will be called synchronously after `async_fn`
        is done *on all ranks*.
      async_fn_kwargs (Tuple): kwargs to pass to `async_fn`.
      preload_fn (Callable): preload function to stage tensors from GPU to Host.
        This should be self-contained with a proper list of arguments with
        `partial`.
      is_frozen (Bool): a flag to indicate this async request can be modified or
        not.
      call_idx (int): index variable used to order async requests for
        synchronization in preloading and writing tensors on the async caller
  """
  ...
  def add_finalize_fn(self, fn: Callable) -> None:
    ...
    Args:
        fn (Callable): function to add to the async request. This function will
          be called *after* existing finalization functions.
...
class AsyncCaller(ABC):
  ...
  Starts process asynchronously and allows checking if all processes on all
  ranks are done.
  """
  ...
  def schedule_async_call(self, async_req: AsyncRequest) -> None:
    ...
    Args:
        async_req (AsyncRequest): `AsyncRequest` object containing to start
          async process
    """
  ...
  def is_current_async_call_done(self, blocking: bool, no_dist: bool) -> bool:
    ...
    Args:
        blocking (bool, optional): if True, will wait until the call is done on
          all ranks. Otherwise, returns immediately if at least one rank is
          still active. Defaults to False.
  ...
  def sync_all_async_calls(self, is_alive: int) -> bool:
    ...
    Returns:
        bool: True if all ranks are done, False if at least one rank is still
        active.
    """
    ten = torch.tensor(
  ...
  def close(self):
    ...
    logger.info(
        f"AsyncCaller: {torch.distributed.get_rank()}, Destroying Async Caller"
    )
...
class TemporalAsyncCaller(AsyncCaller):
  ...
  def schedule_async_call(self, async_req: AsyncRequest) -> None:
    ...
    Args:
        async_fn (Callable, optional): async function to call. If None, no
    ...
    init_time = time()
    logger.debug(
        f"rank: {rank}, takes {init_time - self.start_time} to schedule async"
        " ckpt "
    )

  def is_current_async_call_done(
      self, blocking: bool = False, no_dist: bool = False
  ) -> bool:
    """Check if async save is finished on all ranks.
...
class PersistentAsyncCaller(AsyncCaller):
  ...
  def schedule_async_call(self, async_req: AsyncRequest) -> None:
    ...
    Args:
        async_fn (Callable, optional): async function to call. If None, no
          process will be started.
        async_req (AsyncRequest): `AsyncRequest` object containing to schedule a
    ...
    if self.process is None:
      ctx = mp.get_context("spawn")
      logger.info(
          f"PersistentAsyncCaller: {torch.distributed.get_rank()}, Starting"
          " Async Caller"
      )
      ...
      self.process.start()
      logger.info(
          f"PersistentAsyncCaller: {torch.distributed.get_rank()}, Started"
          " Async Caller"
      )
    ...
    logger.debug(
        f"rank: {torch.distributed.get_rank()}, put {async_req.call_idx}"
    )
    ...
    init_time = time()
    logger.debug(
        f"rank: {torch.distributed.get_rank()}, takes"
        f" {init_time - self.start_time} to schedule async ckpt "
    )
  ...
  def close(self):
    logger.info(
        f"PersistentAsyncCaller: {torch.distributed.get_rank()}, Destroying"
        " Async Caller"
    )
    if self.process:
      self.queue.put("DONE")
  ...
  def async_loop(
    ...
    When the `async_fn` from the request` is completed (background saving is
    done),
    it puts a integer value to `comp_q` to notify the trainer the completion.

    Args:
        rank (int): the rank of the trainer where the persistent worker is
          created.
        queue (mp.JoinableQueue): the main queue used to receive `AsyncRequest
          from the training rank
        preload_q (mp.JoinableQueue): a queue to inform trainer that preloading
          of tensors from GPU to Host or dedicated location is completed
        comp_q (mp.Queue): a queue to inform the training rank the completion of
          scheduled async checkpoint request
        log_level (int, Optional): an integer to set log-level in this spawned
          process to get aligned with the training rank's logging level
    """
    logger = logging.getLogger(__name__)
    logger.setLevel(log_level)
    logger.info(
        f"PersistentAsyncCaller: persistent ckpt worker for {rank} has started"
    )
    while True:
      item = queue.get()
    ...
    logger.info(
        f"PersistentAsyncCaller: persistent ckpt worker for {rank}  has"
        " terminated"
    )
...
class _ActiveAsyncRequest(NamedTuple):
  ...
  Args:
      ...
      async_caller (DistributedAsyncCaller): async caller instance that
        represents the async process handling the async request
...
class AsyncCallsQueue:
  ...
  def schedule_async_request(self, async_request: AsyncRequest) -> int:
    ...
    self.async_calls.append(
        _ActiveAsyncRequest(self.call_idx, async_caller, finalize_fns)
    )
    return self.call_idx

  def maybe_finalize_async_calls(
      self, blocking=False, no_dist=False
  ) -> List[int]:
...
# Code below is copied from `megatron.core.dist_checkpointing.utils.py`
...
def logger_stack(
  ...
  Temporarily pushes a logger and/or name onto their respective stacks, allowing
  hierarchical
  logging and contextual logger usage. Ensures the logger stack is restored
  afterward.

  Args:
      name (str, optional): Name to add to the logger stack. Defaults to None.
      current_logger (logging.Logger, optional): Logger to use. Defaults to the
        last logger in the stack or a fallback if none exist.

  Yields:
      Tuple[str, logging.Logger]: A tuple with the concatenated logger name
      stack and
...
def debug_time(
  ...
  Args:
      ...
      logger (logging.Logger, optional): Logger for output. Defaults to the
        lowest logger.
      threshold (float, optional): Minimum time (seconds) to log. Skips logging
        if faster.
      level (int, optional): Logging level. Defaults to DEBUG if `threshold` is
        unset; WARNING otherwise.
  """
